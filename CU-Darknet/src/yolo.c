...
#endif
/* parses filename from path and removes extension
 * and returns a concatenated string of descriptor and filename
 * Inputs: char * to path, char * to descriptor
 * Outputs: char * to filename
 */
char * fname(char * s, char * d)
{
	char *ext;
	char *f;
	int len = 0;
	...
	f = strrchr(s,'/');
	f++;
	len = strlen(f)-strlen(ext);
	file = malloc(len+1);
	strncpy(file,f,len);
	file[len]=0;
	
	//concatenates filename and descriptor
	fout = (char*)malloc(strlen(file) + strlen(d) + 1);
	strcpy(fout,d);
	strcat(fout,file);
...
void test_yolo(char *cfgfile, char *weightfile, char *filename, float thresh)
{
    ...
    while(1){
        ...
        draw_detections(im, l.side*l.side*l.n, thresh, boxes, probs, voc_names, voc_labels, 20);
	/*Code for parsing pathname for batch processin*/
	//parse path to filename
	char * str1 = "predictions_";//can be changed to desired name
	char * str2 = "resized_";//can be changed to desired name
	char * fout1 =  fname(input,str1);
        save_image(im, fout1);
        show_image(im, fout1);//"predictions"
	char * fout2 = fname(input,str2); 
        show_image(sized, fout2);//2nd param was "resized"
	free(fout1);
	free(fout2);
	/*End of batch processin modifications*/
        free_image(im);
        free_image(sized);
